...
/// </summary>
public class PlayerFlightControls_old : MonoBehaviour
{
	...
	const float _enginePower = 800;//400;
	const float _jetPower = 1000;//550;
	...
	void Start()
	{
		GetComponent<Rigidbody>().centerOfMass = new Vector3(0, -0.25f, 0f);
	...
	{
		var relativeVel = GetComponent<Rigidbody>().transform.worldToLocalMatrix * GetComponent<Rigidbody>().velocity;
		...
		GetComponent<Rigidbody>().AddRelativeTorque(new Vector3(pitch * deltaMultiplier, 0, 0));
		
		var yaw = _yawControl * _yawStrength * controlCurve;
		GetComponent<Rigidbody>().AddRelativeTorque(new Vector3(0, yaw * deltaMultiplier, 0));
		
		var roll = _rollControl * _rollStrength * controlCurve;
		GetComponent<Rigidbody>().AddRelativeTorque(new Vector3(0, 0, roll * deltaMultiplier));
		...
		GetComponent<Rigidbody>().AddRelativeForce(0, 0, brakeForce * deltaMultiplier);
		...
		// lift
		var liftZero = GetComponent<Rigidbody>().mass * Physics.gravity.magnitude * 1.05f; // enough lift for takeoff / level flight
		var lift = LiftBySpeed.Evaluate(speed / _liftTopSpeed) * liftZero;
		GetComponent<Rigidbody>().AddRelativeForce(new Vector3(0, lift * deltaMultiplier, 0));
		...
		GetComponent<Rigidbody>().AddRelativeForce(dragVector * deltaMultiplier);
		
		// extra lift due to Angle Of Attack
		var aoa = Vector3.Angle(transform.forward.IgnoreX(), GetComponent<Rigidbody>().velocity.IgnoreX());
		if (aoa < 90)
		{
			if (aoa > 45)
				aoa = 45 + (45 - aoa);
			...
			GetComponent<Rigidbody>().AddRelativeForce(aoaVector * deltaMultiplier);
		}
		// weathervaning
		...
		GetComponent<Rigidbody>().AddRelativeTorque(correctionVector * deltaMultiplier);
	}
